...
fetch("./data.json")
  ...
  .then((data) => {
    ...
    // on select
    map.on({
      "selectarea:selected": (evt) => {
        console.log("selectarea:selected", evt);
        L.Util.requestAnimFrame(() => {
          map.eachLayer((pointLayer) => {
            if (pointLayer instanceof L.CircleMarker) {
              pointLayer.setStyle({
                color: evt.bounds.contains(pointLayer.getLatLng())
                  ? "#0f0"
                  : "#f00",
              });
            }
        ...
        result.innerHTML = `<pre>${evt.bounds
          .toBBoxString()
          .split(",")
          .join(",\n")}</pre>`;
      },
      "selectarea:toggled": updateButton,
    });
